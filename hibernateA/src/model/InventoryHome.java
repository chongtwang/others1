...
public class InventoryHome {
	...
	public Inventory merge(Inventory detachedInstance) {
		...
		try {
			Inventory result = (Inventory) sessionFactory.getCurrentSession()
	...
	public Inventory findById(java.lang.Integer id) {
		...
		try {
			Inventory instance = (Inventory) sessionFactory.getCurrentSession()
					.get("Inventory", id);
			if (instance == null) {
	...
	public List<Inventory> findByExample(Inventory instance) {
		...
		try {
			List<Inventory> results = (List<Inventory>) sessionFactory
					.getCurrentSession().createCriteria("Inventory")
