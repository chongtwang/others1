...
public class StaffListHome {
	...
	public StaffList merge(StaffList detachedInstance) {
		...
		try {
			StaffList result = (StaffList) sessionFactory.getCurrentSession()
	...
	public List<StaffList> findByExample(StaffList instance) {
		...
		try {
			List<StaffList> results = (List<StaffList>) sessionFactory
					.getCurrentSession().createCriteria("StaffList")
