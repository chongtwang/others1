...
public class EmpHome {
	...
	public Emp merge(Emp detachedInstance) {
		...
		try {
			Emp result = (Emp) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Emp findById(EmpId id) {
		log.debug("getting Emp instance with id: " + id);
		try {
			Emp instance = (Emp) sessionFactory.getCurrentSession().get("Emp",
					id);
			if (instance == null) {
	...
	public List<Emp> findByExample(Emp instance) {
		...
		try {
			List<Emp> results = (List<Emp>) sessionFactory.getCurrentSession()
					.createCriteria("Emp").add(create(instance)).list();
