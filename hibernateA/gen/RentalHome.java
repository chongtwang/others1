...
public class RentalHome {
	...
	public Rental merge(Rental detachedInstance) {
		...
		try {
			Rental result = (Rental) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Rental findById(java.lang.Integer id) {
		...
		try {
			Rental instance = (Rental) sessionFactory.getCurrentSession().get(
					"Rental", id);
			if (instance == null) {
	...
	public List<Rental> findByExample(Rental instance) {
		...
		try {
			List<Rental> results = (List<Rental>) sessionFactory
					.getCurrentSession().createCriteria("Rental")
