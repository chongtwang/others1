...
public class SalesByStoreHome {
	...
	public SalesByStore merge(SalesByStore detachedInstance) {
		...
		try {
			SalesByStore result = (SalesByStore) sessionFactory
	...
	public SalesByStore findById(SalesByStoreId id) {
		...
		try {
			SalesByStore instance = (SalesByStore) sessionFactory
					.getCurrentSession().get("SalesByStore", id);
			if (instance == null) {
	...
	public List<SalesByStore> findByExample(SalesByStore instance) {
		...
		try {
			List<SalesByStore> results = (List<SalesByStore>) sessionFactory
					.getCurrentSession().createCriteria("SalesByStore")
